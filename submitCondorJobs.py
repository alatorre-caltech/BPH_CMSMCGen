...
# Example: ./submitCondorJobs.py mu --events 150000 --jobs 1000 -f --pileup c2
if __name__ == "__main__":
    ...
    parser.add_argument ('-f', '--force_production', action='store_true', help='Proceed even if the directory is already existing')
    ...
    fsub.write('\n')
    exec_args = str(args.events)+' '+str(args.seed)+' $(ProcId) '+args.process+' '+outdir+' '+args.cmssw_loc+' '+str(args.pileup)+' '+str(args.cpu)
    ...
    cmd += ' -batch-name ' + '_'.join(['gen', args.process, 'PU'+str(args.pileup)])
